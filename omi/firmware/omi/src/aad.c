...
static void preroll_flush(void)
{
    ...
    uint8_t start = (vad_preroll_wr + VAD_PREROLL_FRAMES - vad_preroll_cnt) % VAD_PREROLL_FRAMES;
    for (uint8_t i = 0; i < vad_preroll_cnt; i++) {
        uint8_t idx = (start + i) % VAD_PREROLL_FRAMES;
        int err = codec_receive_pcm(vad_preroll_buf[idx], MIC_BUFFER_SAMPLES);
        if (err) {
            LOG_ERR("Preroll flush frame %u failed: %d", i, err);
            break;
        }
    }
    LOG_INF("VAD: flushed %u pre-roll frame(s)", vad_preroll_cnt);
    preroll_reset();
}
